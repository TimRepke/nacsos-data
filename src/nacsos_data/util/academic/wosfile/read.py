# source: https://raw.githubusercontent.com/rafguns/wosfile/master/wosfile/read.py
import codecs
import logging
import pathlib
...
class Reader:
    def __init__(self, fh: TextIO, **kwargs: Any) -> None:
        self.fh = fh
...
def read(
    ...
    if not isinstance(fname, (str, pathlib.Path)):
        ...
        with open(fname, encoding=encoding) as fh:
            yield from reader_class(fh, **kwargs)  # type: ignore[misc] # FIXME
