# source: https://raw.githubusercontent.com/rafguns/wosfile/master/wosfile/record.py
import re
...
class Record(dict[str, str | list[str]]):
    def __init__(self, wos_data: dict[str, str] | None = None, skip_empty: bool = True) -> None:
    ...
    def parse(self, wos_data: dict[str, str]) -> None:
        ...
        for field_name, value in wos_data.items():
            if self.skip_empty and not value:
                continue
            if is_splittable.get(field_name, False):
    ...
    def record_id(self) -> str:
        ...
        volume = 'V' + self['VL'] if 'VL' in self else None  # type: ignore[operator]
        page = 'P' + self['BP'] if 'BP' in self else None  # type: ignore[operator]
    ...
    def author_address(self) -> Optional[Union[List[str], Dict[str, List[str]]]]:
        try:
            return parse_address_field(self['C1'])  # type: ignore[arg-type]
        except KeyError:
            return None
...
def records_from(fname: Union[str, Iterable[str]], skip_empty: bool = True, **kwargs: Any) -> Iterator[Record]:
