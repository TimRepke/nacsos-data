...
async def main(
    ...
    async with db_engine.session() as session:  # type: AsyncSession
        base_cols, label_cols, df = await wide_export_table(
            session=session, scope_ids=priority.source_scopes, limit=None, project_id=priority.project_id, nql_filter=priority.nql_parsed
        )
    ...
    fig, buscar, frontier = buscar_frontiers(df=df, key=incl_field, fig_params=fig_params, batch_size=buscar_batchsize)
    ...
    est = workload_estimation(df=df, source=incl_field, target=incl_pred_field, recall_targets=None)
    with open(out_path / tab_est, 'w') as f:
        f.write(est)

    logger.info('Creating classification report...')
    test_eval, self_eval = report(df=df, source=incl_field, target=incl_pred_field)
    ...
    fig, buscar = buscar_workload(
        df=df,
        source=incl_field,
        target=incl_pred_field,
        batch_size=buscar_batchsize,
        fig_params=fig_params,
        recall_target=buscar_recall_target,
        bias=buscar_bias,
    ...
    fig, stats = roc_auc(df=df, source=incl_field, target=incl_pred_field, fig_params=fig_params)
    ...
    fig = score_distribution(df=df, source=incl_field, target=incl_pred_field, fig_params=fig_params)
