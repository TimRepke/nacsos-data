...
async def load_vectors_from_database(session: AsyncSession, project_id: str,
                                     vectoriser: TransformerMixin | None = None,
                                     batch_size: int = 500) \
        -> Generator[tuple[list[str], list[str], csr_matrix], None, None]:
...
def load_vectors_from_file(filename: str,
    ...
    for batch in batched(parse_lexis_nexis_file(file_path=filename, fail_on_error=fail_on_error),
...
async def construct_lookup(session: AsyncSession,
    ...
    lexis_ids_db = {r: i for i, r in enumerate(bid for batch_ids, _, _ in db_data for bid in batch_ids)}
    lexis_ids_db_inv = {v: k for k, v in lexis_ids_db.items()}
    ...
    lexis_ids_f = {r: i + offset for i, r in enumerate(bid for batch_ids, _, _ in f_data for bid in batch_ids)}
    ...
    for result, item, source in parse_lexis_nexis_file(file_path=filename, fail_on_error=fail_on_parse_error):
        ...
        if item.text is not None and len(item.text) > MIN_TEXT_LEN:
            ...
            indices, similarities = index.query(vector, k=1)
            index, similarity = indices[0], similarities[0]
            if similarity < max_slop:
                ...
                # See, if we've seen this and saved this aleady in the process
        ...
        if existing_id is None:
            ln2id[item.lexis_id] = item.item_id
