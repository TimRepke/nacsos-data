import uuid
from sqlalchemy import Integer, Float, String, Boolean, DateTime, Enum as SAEnum, ForeignKey, UniqueConstraint, CheckConstraint, Identity
...
class BotAnnotationMetaData(Base):
    ...
    bot_annotation_metadata_id = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4, nullable=False, unique=True, index=True)
    ...
    assignment_scope_id = mapped_column(UUID(as_uuid=True), ForeignKey('assignment_scope.assignment_scope_id'), nullable=True, index=True)
    # (Optional) reference to an annotation scheme used here
    annotation_scheme_id = mapped_column(
        UUID(as_uuid=True), ForeignKey('annotation_scheme.annotation_scheme_id', ondelete='CASCADE'), nullable=True, index=True
    )
    # Additional information for this Bot for future reference
    meta: Mapped[BotMeta] = mapped_column(mutable_json_type(dbtype=JSONB(none_as_null=True), nested=True), nullable=True)
...
class BotAnnotation(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint('num_nonnulls(value_bool, value_int, value_float, value_str, multi_int) = 1', name='bot_annotation_has_value'),
    )

    # Unique identifier for this BotAnnotation
    bot_annotation_id = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4, nullable=False, unique=True, index=True)
    bot_annotation_metadata_id = mapped_column(
        UUID(as_uuid=True), ForeignKey(BotAnnotationMetaData.bot_annotation_metadata_id, ondelete='CASCADE'), nullable=False, index=True
    )
    ...
    parent = mapped_column(UUID(as_uuid=True), ForeignKey('bot_annotation.bot_annotation_id', ondelete='CASCADE'), nullable=True, index=True)
