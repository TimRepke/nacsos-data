...
class Import(Base):
    ...
    import_id = Column(PG_UUID(as_uuid=True), primary_key=True, default=uuid4,
                       nullable=False, unique=True, index=True)  # type: Column[UUID | str]

    # The user who created this import (may be NULL if done via a script)
    user_id = Column(PG_UUID(as_uuid=True), ForeignKey(User.user_id),
                     nullable=True, index=True, primary_key=False)  # type: Column[UUID | str]

    # The project this import is attached to
    project_id = Column(PG_UUID(as_uuid=True), ForeignKey(Project.project_id),
                        nullable=False, index=True, primary_key=False)  # type: UUID | str
    ...
    type = Column(SAEnum(ImportType), nullable=False)  # type: ImportType

    # Date and time when this import was created and when the actual import was triggered
    time_created = Column(DateTime(timezone=True), server_default=func.now())  # type: datetime
    time_started = Column(DateTime(timezone=True), nullable=True)  # type: datetime | None
    time_finished = Column(DateTime(timezone=True), nullable=True)  # type: datetime | None

    # This stores the configuration of the respective import method
    config = Column(mutable_json_type(dbtype=JSONB, nested=True))  # type: ImportConfig
