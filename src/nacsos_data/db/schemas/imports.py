...
m2m_import_item_table = Table(
    ...
    ForeignKeyConstraint(['import_id', 'latest_revision'],
                         ['import_revision.import_id', 'import_revision.import_revision_counter']),
)
...
class Import(Base):
    ...
    revisions: Relationship['ImportRevision'] = relationship('ImportRevision',
                                                             cascade='all, delete')
...
class ImportRevision(Base):
    ...
    __table_args__ = (
        UniqueConstraint('import_id', 'import_revision_counter'),
    )

    # Unique identifier for this import revision
    import_revision_id = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid4,
                                       nullable=False, unique=True, index=True)
