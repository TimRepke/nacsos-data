...
from nacsos_data.util.academic.apis.openalex.solr import wildcards
...
@app.command(help='Download raw data from an academic search API')
def download(
    kind: APIEnum,
...
@app.command(help='Convert raw data format into NACSOS format')
def convert(
    kind: APIEnum,
...
app.command('solr-wildcards', help='Extract all wildcard terms from a query and prepare a ranking of possible replacements')(wildcards)
