...
from nacsos_data.util.export.file import get_author_names, write_csv, write_excel, write_jsonl
...
def exporter(
    ...
    project_id: Annotated[str, typer.Option(help='Project ID')],  # 5a1fc4b5-7eef-4640-b863-7b506cc3dad6
    ...
    bot_annotation_metadata_ids: Annotated[str | None, typer.Option(help='Comma separated Bot Annotation Metadata IDs')] = None,
    assignment_scope_ids: Annotated[str | None, typer.Option(help='Comma separated Assignment Scope IDs')] = None,
    user_ids: Annotated[str | None, typer.Option(help='Comma separated User IDs')] = None,
    labels: Annotated[str | None, typer.Option(help='Comma separated labels to export')] = None,
    nql_filter: Annotated[str, typer.Option(help='NQL query to use as filter')] = 'HAS ANNOTATION',
    ignore_repeat: Annotated[bool, typer.Option(help='Ignore annotation order')] = True,
    ignore_hierarchy: Annotated[bool, typer.Option(help='Ignore annotation hierarchy')] = True,
    max_results: Annotated[int, typer.Option(help='Max results')] = 15000,
    ...
    async def _run() -> Path:
        ...
        bot_annotation_metadata_ids_list = bot_annotation_metadata_ids.split(',') if bot_annotation_metadata_ids is not None else []
        assignment_scope_ids_list = assignment_scope_ids.split(',') if assignment_scope_ids is not None else []
        user_ids_list = user_ids.split(',') if user_ids is not None else []
        # labels_list = labels.split(',') if labels is not None else []
        ...
        src = Path(fp)
        dest = Path('export_now.jsonl')
        dest.parent.mkdir(parents=True, exist_ok=True)

        with open(src, 'rb') as fsrc, open(dest, 'wb') as fdst:
            fdst.write(fsrc.read())

        # optionally remove the temp file after copy
        src.unlink()
